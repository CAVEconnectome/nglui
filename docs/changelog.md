...
- **StateBuilder**: Annotation tags can now be emitted in main Neuroglancer's format, where a tag is an annotation property of `type: "bool"` bound to a `toggleBoolProperty` hotkey, alongside the existing Spelunker format. The encoding is chosen by what the target deployment supports rather than by a version number, since Neuroglancer is not semantically versioned. Pass `capabilities="main"` (tracking google/neuroglancer's main branch) or `capabilities="legacy"` (tracking Spelunker) to `ViewerState` or an `AnnotationLayer` to choose explicitly; left unset, nglui reads `version.json` from the target site and keys on the `git describe` release the build descends from, so forks and rebased branches are recognized without an allowlist. A deployment that cannot be identified is assumed to track main, and warns — set `capabilities="legacy"` or `set_default_capabilities("legacy")` for an unidentifiable deployment that predates bool annotation properties, which would otherwise drop the annotation layer rather than render it. That lookup only happens when a local annotation layer has tags, is cached, and never raises, so offline use is unaffected.
- **StateBuilder**: `set_default_capabilities()` changes the assumed encoding when the target is unknown, and `NGLUI_DISABLE_CAPABILITY_PROBE=1` suppresses the lookup.
- **StateBuilder**: `Capabilities` is a named set rather than a label: `Capabilities.names()` lists what nglui knows, `.enabled` gives the available ones, `capability in capabilities` tests membership, and `Capabilities.from_names([...])` builds sets the `"legacy"`/`"main"` shorthands do not cover — such as a build that understands bool properties but not the tools that toggle them. Provenance is recorded in `source`, which is excluded from equality so a probed set compares equal to an equivalent named one.
- **StateBuilder**: Tag labels become property identifiers under the modern encoding, which Neuroglancer requires to match `^[a-z][a-zA-Z0-9_]*$`. Labels are sanitized the way Neuroglancer's own annotation tab sanitizes typed input, with a single warning per layer listing what changed. `tag_ids` sets identifiers explicitly and `strict_property_ids=True` turns renaming into an error.
- **Parser**: `annotation_property_dictionary()` reports every annotation property in a layer, not only tag-like ones.
...
- **StateBuilder**: Annotation tool bindings were emitted whenever bool properties were used, but the property tools landed upstream three months after the properties themselves. A deployment built in between understands the properties and not the bindings, and an unparseable tool type costs the whole layer, so bindings are now gated on that capability separately.
- **SegmentProperties**: A segment with a missing value in a `tag_bool_cols` column was reported as carrying that tag. `np.flatnonzero` counts NaN as non-zero, so an absent flag read as a set one — silently, and in the direction that invents data.
- **StateBuilder**: A missing value in a tag column no longer raises. `tag_column` with a NaN or None raised `TypeError` when ordering the layer's tag vocabulary, and `tag_bools` holding a NaN or a nullable dtype raised `IndexError` when used as a mask — both ordinary shapes for real data. Missing now means untagged, as do blank strings.
- **StateBuilder**: Non-string tag values were silently never set. Annotations coerce their own tags to strings, but the layer's vocabulary kept the raw value, so a numeric `tag_column` produced a correct-looking property list with every value unset. The vocabulary is coerced to match.
- **StateBuilder**: Non-ASCII tag labels are folded toward ASCII before becoming property identifiers. Accented Latin decomposes (`café` → `cafe`, previously `caf`), and Greek letters are replaced by their names (`β-cell` → `beta_cell`) — without which `α-cell`, `β-cell` and `γ-cell` all sanitized to `cell` and were then disambiguated into `cell`/`cell_2`/`cell_3`, making distinct cell types indistinguishable in output that looked correct. Scripts with no Latin reading still fall back to a generated id, with the original kept in the property description.
- **StateBuilder**: A repeated entry in an annotation layer's `tags` produced output no viewer accepts — two properties sharing an identifier under the bool encoding, and a per-annotation value vector shorter than the property list under the Spelunker one. Repeats are now emitted once, with a warning.
